...
export const FloatingActionBar: React.FC<FloatingActionBarProps> = ({
    ...
    return (
      ...
      `}
        >
            <div className="bg-stone-200/60 backdrop-blur-xl border border-stone-300/50 shadow-lg rounded-full px-2 py-1 flex items-center gap-1 pointer-events-auto max-w-fit">
                <ActionButton
...
const ActionButton: React.FC<ActionButtonProps> = ({ icon, label, onClick, disabled, isActive }) => {
    return (
        <button
            ...
            disabled={disabled}
            className={`
        flex flex-col items-center justify-center min-w-[60px] py-1 px-1 rounded-2xl transition-all duration-200
